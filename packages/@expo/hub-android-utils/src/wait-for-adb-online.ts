...
/** Options for {@link waitForAdbOnline}; both are injectable for testing. */
 ...
 * or `false` once `timeoutMs` has elapsed. Errors from the device lister are
 * swallowed so a transient `adb` hiccup doesn't abort the wait.
 */

...
describe('offline selected device', () => {
  ...
  test('retains inspector values when the client resets and clears them for a different device', async () => {
    ...
    try {
      ...
      const upstreamSections = [AccessibilitySection, LocationSection, PermissionsSection].map(
        (Section) => renderer!.root.findByType(Section)
      );
      await act(() => {
      ...
      for (const section of upstreamSections) {
        expect(renderer!.root.findByType(section.type)).toBe(section);
        expect(section.props.client).toBe(CLIENT);
        expect(section.props.available).toBeFalse();
      }
...
test('disables upstream inspector actions while retaining their offline values', () => {
  const client: DeviceClient = {
    ...CLIENT,
    platform: 'android',
    foregroundApp: { id: 'dev.example.app' },
    permissions: [{ id: 'camera', label: 'Camera', state: 'undetermined' }],
  ...
  for (const Section of [
    ...
    const live = renderToStaticMarkup(<Section client={client} defaultOpen />);
    const offline = renderToStaticMarkup(<Section client={client} available={false} defaultOpen />);
    expect(buttonTags(offline)).toHaveLength(buttonTags(live).length);
    // The section disclosure stays usable, but every host action is disabled.
    expect(buttonTags(offline).filter((tag) => !tag.includes('disabled=""'))).toHaveLength(1);
  }
});

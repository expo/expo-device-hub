...
export async function startGpuExperimentSession(
  ...
  socketPath: string,
  dependencies: GpuSessionDependencies = {},
): Promise<EmuSession> {
  ...
  if (!socketPath.startsWith("/"))
    throw new Error("GPU experiment socket path must be absolute");
  ...
  const meta = {
    deviceName: "GPU capture experiment",
    codecId: "h264",
    width: 0,
    height: 0,
  };
  ...
  const reader = new GpuPacketReader((record) => {
    if (record.flags === 2) {
      ...
      Object.assign(
        meta,
        gpuStreamSize(nativeSize.width, nativeSize.height, maxSize),
      );
      streamFps = options.maxFps || sourceFps;
      socket.write(
        gpuSettingsCommand(maxSize, streamFps, options.bitRate ?? 12_000_000),
        (error) => {
          if (error) fail(error);
        },
      );
  ...
  socket.on("close", () =>
    fail(new Error("Native GPU capture connection closed")),
  );
  ...
  try {
    ...
    control = await (dependencies.startScrcpyControl ?? startScrcpyControl)(
      options,
    );
    if (signal.aborted || failed)
      throw new Error("GPU experiment startup interrupted");
    const writer = new SocketControlWriter(control.controlSocket);
    control.controlSocket.on("error", fail);
    control.proc.on("exit", () =>
      fail(new Error("GPU experiment control process exited")),
    );
      ...
      dispatcher: {
        async dispatchGesture(gesture, _screen, inputSignal) {
          for (const step of compileGesture(gesture, nativeSize).steps) {
            if (step.delayMs > 0)
              await delay(step.delayMs, undefined, { signal: inputSignal });

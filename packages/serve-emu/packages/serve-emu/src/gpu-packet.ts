...
export class GpuPacketReader {
  ...
  push(chunk: Buffer): void {
    ...
    while (this.#pending.length >= GPU_HEADER_BYTES) {
      ...
      if (b.toString("ascii", 0, 4) !== "GPC1")
        throw new Error("Invalid GPU stream magic");
      const length = b.readUInt32BE(4);
      const flags = b.readUInt32BE(16);
      const width = b.readUInt32BE(20),
        height = b.readUInt32BE(24),
        fps = b.readUInt32BE(28);
      if (
        length > MAX_GPU_PACKET_BYTES ||
        flags > 3 ||
      ...
      const record = {
        pts: b.readBigUInt64BE(8),
        flags,
        width,
        height,
        fps,
        data: Buffer.from(
          b.subarray(GPU_HEADER_BYTES, GPU_HEADER_BYTES + length),
        ),
      };
...
export function splitGpuAccessUnit(data: Buffer): {
  ...
  for (let i = 0; i + 3 < data.length; i++) {
    ...
    const size =
      data[i + 2] === 1 ? 3 : data[i + 2] === 0 && data[i + 3] === 1 ? 4 : 0;
    if (!size || i + size >= data.length) continue;
    starts.push({ offset: i, header: i + size });
    i += size - 1;
  }
  if (!starts.length || starts[0]!.offset !== 0)
    throw new Error("GPU packet is not Annex-B H.264");
  ...
  for (let i = 0; i < starts.length; i++) {
    ...
    const nal = data.subarray(
      start.offset,
      starts[i + 1]?.offset ?? data.length,
    );
  ...
  return {
    sps,
    pps,
    data: hasPicture ? Buffer.concat(picture) : Buffer.alloc(0),
    isIdr,
  };
}
...
export function gpuStreamSize(width: number, height: number, maxSize: number) {
  ...
  const edge = Math.max(width, height),
    target = maxSize > 0 ? Math.min(maxSize, edge) : edge;
  const size = {
    width: Math.floor((width * target) / edge / 2) * 2,
    height: Math.floor((height * target) / edge / 2) * 2,
  };
  if (size.width < 2 || size.height < 2)
    throw new Error("GPU stream size is too small for H.264");
  return size;
}

export function gpuSettingsCommand(
  maxSize: number,
  fps: number,
  bitRate: number,
): Buffer {
  ...
  command[0] = 83; // S
  command.writeUInt32BE(maxSize, 1);
  command.writeUInt32BE(fps, 5);
  command.writeUInt32BE(bitRate, 9);
  return command;
}

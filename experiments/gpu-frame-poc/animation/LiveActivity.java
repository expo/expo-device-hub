...
public class LiveActivity extends Activity {
  ...
  @Override public void onCreate(Bundle state) {
    ...
    window.preferredRefreshRate = 120;
    getWindow().setAttributes(window);
    final float unit = getResources().getDisplayMetrics().density / 1.5f;
    final int padding = Math.round(20 * unit);
    ...
    root.setOnApplyWindowInsetsListener((view, insets) -> {
      ...
      view.setPadding(padding, safe.top + padding, padding, safe.bottom + padding);
      return insets;
    });
    ...
    View motion = new View(this) {
      ...
      @Override protected void onDraw(Canvas canvas) {
        ...
        float x = (frame * 7) % Math.max(1, getWidth() / unit);
    ...
    root.addView(motion, new LinearLayout.LayoutParams(-1, Math.round(180 * unit)));
    ...
    for (int i = 1; i <= 30; i++) {
      ...
      row.setText("Swipe test — row " + i); row.setTextSize(20);
      row.setPadding(Math.round(12 * unit), Math.round(18 * unit), Math.round(12 * unit), Math.round(18 * unit));
      rows.addView(row);
    }

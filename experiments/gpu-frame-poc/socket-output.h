...
// height(u32), fps(u32). Flags: 1=IDR, 2=hello. Payload is one AVPacket.
// Only the encoder worker accesses client; the render callback never writes.
struct SocketOutput {
    int listener=-1,client=-1; std::string path; bool needsKey=true;
    ...
    void pollCommands(int w,int h,int fps) {
        ...
        char commands[64];ssize_t n=::recv(client,commands,sizeof(commands),MSG_DONTWAIT);
